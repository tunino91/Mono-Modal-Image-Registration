...

{
  ...
  if( argc < 4 )
    ...
    ReaderType::Pointer ReadFixedChest = ReaderType::New();

    ReaderType::Pointer ReadFixedLung = ReaderType::New();

    ReaderType::Pointer ReadMovingChest = ReaderType::New();

    ReaderType::Pointer ReadMovingLung = ReaderType::New();

    WriterType::Pointer WriteFixedMasked = WriterType::New();

    WriterType::Pointer WriteMovingMasked = WriterType::New();

    

    //Parameters

    ReadFixedChest->SetFileName( argv[1] ); // Chest fixed

    ReadFixedLung->SetFileName(argv[2]);   // Lung fixed

    ReadMovingChest->SetFileName( argv[3] ); // Chest moving

    ReadMovingLung->SetFileName(argv[4]);   // Lung moving

    WriteFixedMasked->SetFileName( argv[5] ); // Masked fixed

    WriteMovingMasked->SetFileName( argv[6] ); // Masked moving
...

    {
        ...
        ReadFixedChest->Update();

        ReadFixedLung->Update();

        ReadMovingChest->Update();

        ReadMovingLung->Update();
    ...
    MaskFilterType::Pointer FixedMaskFilter = MaskFilterType::New();

    FixedMaskFilter->SetInput(ReadFixedChest->GetOutput());

    FixedMaskFilter->SetMaskImage(ReadFixedLung->GetOutput());

    FixedMaskFilter->Update();

    

    MaskFilterType::Pointer MovingMaskFilter = MaskFilterType::New();

    MovingMaskFilter->SetInput(ReadMovingChest->GetOutput());

    MovingMaskFilter->SetMaskImage(ReadMovingLung->GetOutput());
    ...
    histogramMatching->SetInput(MovingMaskFilter->GetOutput());

    histogramMatching->SetReferenceImage( FixedMaskFilter->GetOutput() );

    histogramMatching->SetNumberOfHistogramLevels(150);

    histogramMatching->SetNumberOfMatchPoints(15);

    histogramMatching->Update();

    

    WriteFixedMasked->SetInput(FixedMaskFilter->GetOutput());

    WriteMovingMasked->SetInput(histogramMatching->GetOutput());
...

    {
        ...
        WriteFixedMasked->Update();

        WriteMovingMasked->Update();
  ...
  fixedImageReader->SetFileName(  argv[5] ); // Masked fixed

  movingImageReader->SetFileName( argv[6] ); // Masked moving
  ...
  if( argc > 11 )
    ...
    metric->SetUseExplicitPDFDerivatives( atoi( argv[11] ) );
  ...
  if( argc > 12 )
    ...
    metric->SetUseCachingOfBSplineWeights( atoi( argv[12] ) );
  ...
  if( argc > 15 )
    ...
    optimizer->SetMaximumStepLength( atof( argv[16] ) );
  ...
  if( argc > 16 )
    ...
    optimizer->SetNumberOfIterations( atoi( argv[17] ) );
  ...
  writer3->SetInput( difference->GetOutput() );
  ...
  if( argc > 10 )
    ...
    fieldWriter->SetFileName( argv[10] );
